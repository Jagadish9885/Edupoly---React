...
function App() {
  return (
    <div className="border border-2 border-success p-2">
      <NavBar></NavBar>
      <div className="border border-2 d-flex flex-wrap m-2 p-2 justify-content-center bg-secondary bg-gradient">
       {
        products.map((prod)=>{
          return (
            <Products key={prod.id} id={prod.id} descr={prod.description} price={prod.price}
            rating={prod.rating} image={prod.thumbnail} 
            >{prod.title}</Products>
          )
        })
       }
